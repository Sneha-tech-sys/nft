...
// Assuming you have a type definition for NFT data
interface NFT {
  id: string;
  name: string;
  image: string;
...
const NFTTierSlider = ({
  ...
  // Filter NFTs from the same tier excluding the current NFT
      ...
      <Slider ref={sliderRef} {...sliderSettings}>
        {sameTierNFTs.map((item) => (
          <div key={item.id} className="px-2">
            <Link
              to={`/nft-tier/${item.id}`} // Changed from /nft-tier to /nft to match your details page route
